...
pub mod claim_grant_reducer;
pub mod grant_secret_reducer;
pub mod inference_endpoint_table;
pub mod inference_endpoint_type;
pub mod prune_expired_reducer;
pub mod register_endpoint_reducer;
pub mod remove_endpoint_reducer;
pub mod revoke_all_for_agent_reducer;
pub mod revoke_secret_reducer;
pub mod secret_grant_type;
pub mod update_health_reducer;

pub use claim_grant_reducer::claim_grant;
pub use grant_secret_reducer::grant_secret;
pub use inference_endpoint_table::*;
pub use inference_endpoint_type::InferenceEndpoint;
pub use prune_expired_reducer::prune_expired;
pub use register_endpoint_reducer::register_endpoint;
pub use remove_endpoint_reducer::remove_endpoint;
pub use revoke_all_for_agent_reducer::revoke_all_for_agent;
pub use revoke_secret_reducer::revoke_secret;
pub use secret_grant_type::SecretGrant;
pub use update_health_reducer::update_health;
...
impl __sdk::Reducer for Reducer {
    fn reducer_name(&self) -> &'static str {
        match self {
            Reducer::ClaimGrant { .. } => "claim_grant",
            Reducer::GrantSecret { .. } => "grant_secret",
            Reducer::PruneExpired { .. } => "prune_expired",
            Reducer::RegisterEndpoint { .. } => "register_endpoint",
            Reducer::RemoveEndpoint { .. } => "remove_endpoint",
            Reducer::RevokeAllForAgent { .. } => "revoke_all_for_agent",
            Reducer::RevokeSecret { .. } => "revoke_secret",
            Reducer::UpdateHealth { .. } => "update_health",
            _ => unreachable!(),
        }
    }
    ...
    fn args_bsatn(&self) -> Result<Vec<u8>, __sats::bsatn::EncodeError> {
        match self {
            Reducer::ClaimGrant {
                agent_id,
                secret_key,
            } => __sats::bsatn::to_vec(&claim_grant_reducer::ClaimGrantArgs {
            ...
            Reducer::GrantSecret {
                ...
                granted_at,
                expires_at,
            } => __sats::bsatn::to_vec(&grant_secret_reducer::GrantSecretArgs {
                agent_id: agent_id.clone(),
                secret_key: secret_key.clone(),
            ...
            Reducer::PruneExpired { now } => {
                __sats::bsatn::to_vec(&prune_expired_reducer::PruneExpiredArgs { now: now.clone() })
            }
            Reducer::RegisterEndpoint {
                id,
                url,
                provider,
                model,
                requires_auth,
                secret_key,
            } => __sats::bsatn::to_vec(&register_endpoint_reducer::RegisterEndpointArgs {
                id: id.clone(),
                url: url.clone(),
                provider: provider.clone(),
                model: model.clone(),
                requires_auth: requires_auth.clone(),
                secret_key: secret_key.clone(),
            }),
            Reducer::RemoveEndpoint { id } => {
                __sats::bsatn::to_vec(&remove_endpoint_reducer::RemoveEndpointArgs {
                    id: id.clone(),
                })
            }
            Reducer::RevokeAllForAgent { agent_id } => {
                __sats::bsatn::to_vec(&revoke_all_for_agent_reducer::RevokeAllForAgentArgs {
                    agent_id: agent_id.clone(),
                })
            }
            Reducer::RevokeSecret {
                agent_id,
                secret_key,
            } => __sats::bsatn::to_vec(&revoke_secret_reducer::RevokeSecretArgs {
                agent_id: agent_id.clone(),
                secret_key: secret_key.clone(),
            }),
            Reducer::UpdateHealth {
                id,
                status,
                checked_at,
            } => __sats::bsatn::to_vec(&update_health_reducer::UpdateHealthArgs {
                id: id.clone(),
                status: status.clone(),
...
#[doc(hidden)]
pub struct DbUpdate {
    inference_endpoint: __sdk::TableUpdate<InferenceEndpoint>,
}
    ...
    fn try_from(raw: __ws::v2::TransactionUpdate) -> Result<Self, Self::Error> {
        ...
        for table_update in __sdk::transaction_update_iter_table_updates(raw) {
            match &table_update.table_name[..] {
                "inference_endpoint" => db_update
                    .inference_endpoint
                    .append(inference_endpoint_table::parse_table_update(table_update)?),
...
impl __sdk::DbUpdate for DbUpdate {
    fn apply_to_client_cache(
        ...
        diff.inference_endpoint = cache.apply_diff_to_table::<InferenceEndpoint>(
            "inference_endpoint",
...
pub struct AppliedDiff<'r> {
    inference_endpoint: __sdk::TableAppliedDiff<'r, InferenceEndpoint>,
...
impl<'r> __sdk::AppliedDiff<'r> for AppliedDiff<'r> {
    fn invoke_row_callbacks(
        ...
        callbacks.invoke_table_row_callbacks::<InferenceEndpoint>(
            "inference_endpoint",
            &self.inference_endpoint,
            event,
        );
...
impl __sdk::SpacetimeModule for RemoteModule {
    ...
    fn register_tables(client_cache: &mut __sdk::ClientCache<Self>) {
        inference_endpoint_table::register_table(client_cache);
    }
    const ALL_TABLE_NAMES: &'static [&'static str] = &["inference_endpoint"];
}
