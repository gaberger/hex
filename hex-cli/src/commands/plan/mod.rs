...
async fn reconcile_plan(file: &str, update: bool, audit: bool) -> anyhow::Result<()> {
    ...
    let mut evidence_map: std::collections::HashMap<String, reconcile_evidence::TaskEvidence> = std::collections::HashMap::new();
    ...
    for task in &tasks_to_check {
        ...
        // ADR-2604142200: strict AND-based evidence verification
    ...
    if update && (done_count > 0 || !demoted_ids.is_empty()) {
        ...
        if let Some(steps) = raw.get_mut("steps").and_then(|v| v.as_array_mut()) {
            for step in steps.iter_mut() {
                ...
                if done_ids.contains(id.as_str()) && step.get("status").and_then(|v| v.as_str()) != Some("done") {
            ...
            for phase in phases.iter_mut() {
                if let Some(tasks) = phase.get_mut("tasks").and_then(|v| v.as_array_mut()) {
                    for task in tasks.iter_mut() {
                        ...
                        if done_ids.contains(id.as_str()) && task.get("status").and_then(|v| v.as_str()) != Some("done") {
...
fn evidence_snapshot_json(ev: &reconcile_evidence::TaskEvidence) -> serde_json::Value {
    let files_found: Vec<String> = ev.files_exist.iter()
        .filter(|(_, exists)| *exists)
        .map(|(p, _)| p.display().to_string())
        .collect();
    let symbols_matched: Vec<String> = ev.symbol_hits.iter()
        .map(|(sym, _)| sym.clone())
        .collect();
    serde_json::json!({
