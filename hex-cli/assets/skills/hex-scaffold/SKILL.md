---
name: hex-scaffold
description: Scaffold a hexagonal project. Use when the user asks to "create a hex project", "scaffold", "new ports and adapters project", or "hex init".
---
...
```bash
hex init <dir> --scaffold --lang <rust|go|ts>
```
...
```bash
hex scaffold "<one-sentence description>" --target <dir> --lang <rust|go|ts> --grade A
```
...
a violation. Run `hex do` with the failing check as the evidence command.
