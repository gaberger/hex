# Plan: build a key-value store REST API in rust with axum

> Generated by `hex dev` — hexagonal architecture development pipeline

## Overview

Plan: build a key-value store REST API in rust with axum — built with hexagonal (ports & adapters) architecture in Rust.
