# Plan: build a simple Go REST API with gin framework that has CRUD endpoints for todos

> Generated by `hex dev` — hexagonal architecture development pipeline

## Overview

Plan: build a simple Go REST API with gin framework that has CRUD endpoints for todos — built with hexagonal (ports & adapters) architecture in Go.
