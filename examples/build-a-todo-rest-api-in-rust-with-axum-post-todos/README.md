# Plan: build a todo REST API in Rust with axum: POST /todos to create, GET /todos to list, DELETE /todos/:id to remove. In-memory storage, JSON responses.

> Generated by `hex dev` — hexagonal architecture development pipeline

## Overview

Plan: build a todo REST API in Rust with axum: POST /todos to create, GET /todos to list, DELETE /todos/:id to remove. In-memory storage, JSON responses. — built with hexagonal (ports & adapters) architecture in Rust.
