...
| **hex-adapter** | Architecture enforcement runtime — the component that must be present (locally or remotely) on any system running hex development agents. Enforces hexagonal architecture through skills, hooks, ADRs, workplans, HexFlo dispatchers, and agent definitions. | "hex-agent" (legacy name, being renamed) |
