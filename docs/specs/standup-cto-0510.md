...
2. **`hex-nexus/src/tools/workplan_emit.rs`** (275 lines) — `workplan_emit(slug, feature, adr, phases[])` tool. Closes ADR→workplan auto-bridge gap. Enforces schema (ADR-2604142201 `hex plan reconcile` evidence: each task must declare `files[]`). Emits `docs/workplans/wp-<slug>.json` via `proposed_action(file_write)`. Auto-approved. Grounded: `repo_read hex-nexus/src/tools/workplan_emit.rs` line 1-275.
