...
- **References** in workplan JSON `adr` fields and operator notes use legacy `ADR-2026-03-22-1500` format because that's what tooling emits
- **Ambiguity**: given a legacy ID like `ADR-2026-03-22-1500`, humans cannot determine the slug without filesystem lookup
...
5. Verify via `cargo check` and integration test `hex adr search ADR-2026-03-22-1500` (P3.1)
...
- **Alias conflicts**: if two ADRs have same timestamp (collision handled by ADR-2026-03-22-1500 via alpha suffix like `2603221500a`)
...
- `hex-cli`: add `serde_json` dep (already present) + `regex` crate for canonical ID pattern matching (already in Cargo.toml per ADR-2026-04-14-2200)
...
- ADR-2026-03-22-1500: Timestamp-Based ADR Numbering (original decision)
- ADR-2026-05-08-2500: Typed Tool Library + SOP Execution (defines tool contract)
