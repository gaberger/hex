...
**Status:** Accepted
**Implementation-Present:** 2026-05-12 by auto-scan — evidence: hex-cli/assets/context-templates/tools/web-search.md
**Date:** 2026-04-14
...
- **Standalone mode** (ADR-2026-04-11-2000) explicitly targets environments without Claude Code. In that configuration, agents have *no* web access — context-mode is Claude-only.
...
- Existing pattern for third-party I/O is well established: inference routes through `InferencePort` with multiple adapters (Ollama, OpenAI, Anthropic, OpenRouter) and API keys flow via the `secret-grant` WASM module (ADR-2026-03-26-1000). Web search is structurally identical.
...
**Positive:**
- Standalone mode gains web access — ADR-2026-04-11-2000 was blocked on this for research-tier tasks.
- Single audit trail for all web I/O — currently invisible to dashboard and ADR-2026-04-07-1300 audit-trail wiring.
...
**Mitigations:**
- Each provider adapter has contract tests against recorded fixtures (like inference adapters, ADR-2026-03-23-1600).
...
- ADR-2026-03-26-1000: Secure inference and secrets (secret-grant pattern)
- ADR-2026-04-07-1300: Tool-call observability (audit trail pattern)
- ADR-2026-04-11-2000: Standalone dispatch (the mode this unblocks)
