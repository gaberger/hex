...
| `remote-shell` (ADR-2026-04-14-1200) | Agent PATCHed status back with exit 0. |
...
- Emit `brain.lease_expired` event (priority 2 notification via ADR-2026-04-14-1100)
...
- **Move brain entirely into HexFlo**: obviates brain queue. Rejected for this ADR — too large a change; the hex-sched rename (ADR-2026-04-15-0000) just landed and stability matters.
