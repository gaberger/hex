# ADR-2026-04-27-1200: Architectural-health detectors — the improver interrogates domain → port → adapter

**Status:** Accepted
**Implementation-Present:** 2026-05-12 by auto-scan — evidence: hex-cli/assets/improver/detectors.toml
**Date:** 2026-04-27
...
**Depends on:** ADR-2026-04-26-1500 (substrate), ADR-2026-04-26-1311 (six-layer governance + PortTelemetry), ADR-001 (hexagonal architecture), ADR-2026-03-28-3000 (Rust workspace boundary analysis)
<!-- ADR-2026-04-27-1100 (sched improver) was scoped but never written; the dependency was satisfied by ADR-2026-04-15-0000 and successors (see Note above). Removed from Depends-on to clear the dangling reference. -->
...
|---|---|---|
| Domain → port import discipline | `hex analyze .` (tree-sitter, ADR-2026-03-28-3000) | shipped |
...
| `substrate_swap_starvation` | Zero swap_ticket entries promoted in 30 days | `swap_ticket` STDB | "substrate's hot-swap value is unused → propose first-swap experiment or scope-down ADR-2026-04-26-1500" |
...
Per ADR-2026-04-14-2243, detectors are TOML rows in `hex-cli/assets/improver/detectors.toml`. Adding a new detector = adding a row + ensuring the referenced CLI command emits JSON. No Rust changes for new detectors.
