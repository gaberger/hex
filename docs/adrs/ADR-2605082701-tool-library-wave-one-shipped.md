# ADR-2605082701 — tool-library-wave-one-shipped
...
We ship wave one consisting of `cargo_check`, `repo_grep`, and `adr_draft` as the foundational tool library for hex-nexus orchestration. These three tools provide the minimal viable coverage for grounding, verification, and artifact production in the SOP (ADR-2605082500) workflow: GROUND → REASON → DECIDE → ACT.
