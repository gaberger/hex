...
- **Canonical (new)**: `ADR-260322-timestamp-ADR-numbering` (YYYY-MMDD-slug filename convention)
...
```json
{
  "2603221500": "ADR-260322-timestamp-ADR-numbering",
  "2605082500": "ADR-260508-typed-tool-library-and-sop-execution"
}
```
...
- **Both formats valid forever**: legacy `ADR-2603221500` and canonical `ADR-260322-timestamp-ADR-numbering` both resolve to same file
...
3. `hex adr search ADR-2603221500` and `hex adr search ADR-260322-timestamp-ADR-numbering` both resolve to same ADR file
