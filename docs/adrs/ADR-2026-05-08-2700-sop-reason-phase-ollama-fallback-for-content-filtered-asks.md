...
The SOP REASON phase (ADR-2026-04-14-2300) currently hard-codes OpenRouter as the inference provider in `reason_via_openrouter()`. When a 403 content-filter response occurs, the SOP fails immediately with no retry—wasting the GROUND phase work and forcing the operator to manually rephrase the request.
