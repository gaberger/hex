...
Secret grants follow ADR-2026-03-26-1000: the spawning session must hold a `hex secrets grant` before secrets are injected.

### 6. Model selection — any model, no Claude Code dependency

hex-agent selects the LLM for each inference call via the existing inference gateway (ADR-2026-03-27-1000):
...
- ADR-2026-03-26-1000: Secure inference and secrets
