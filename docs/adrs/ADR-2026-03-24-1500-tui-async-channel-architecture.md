...
The hex dev TUI uses ratatui but blocks the render loop during inference calls (5-40s). ADR-2026-03-24-1430 added a two-tick workaround that shows the phase name before blocking, but the UI is still frozen during execution — no progress updates, no key handling, no animations.
