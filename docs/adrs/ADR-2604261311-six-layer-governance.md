# ADR-2604261311: Six-layer governance — adversarial swarms, judges, and structural opposition to accretion
...
Most quarters this produces small adjustments. Periodically it produces something the size of ADR-2604261303 ("the thing you have isn't the thing you wanted, here's the path back"). The ritual is the only mechanism that catches a system drifting away from its founding goals over months.
...
- ADR-2604261303 — IModelProvider port + crate split (the *what* — this ADR is the *how to avoid building it again the same way*)
