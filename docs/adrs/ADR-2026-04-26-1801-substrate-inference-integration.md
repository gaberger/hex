...
**Supersedes:** None — this is a clarifying ADR for ADR-2026-04-26-1500's first port migration.
...
| No substrate intercept of inference at all | Defeats the substrate's first-port pilot; blocks ADR-2026-04-26-1500 P10 (second-port migration justification) |

---

## Decision

**The substrate intercepts inference at the strategy level.** A bound `IInferencePort` adapter is a *whole inference strategy* — `InferenceRouterAdapter`, an alternative `InferenceRouterAdapter` configured with different RL state, a hardcoded fallback, or a generated adapter (per ADR-2026-04-26-1500 C4). The substrate swaps strategies; per-call routing within a strategy stays the strategy's responsibility.
...
- `IAdapterGenerator` (ADR-2026-04-26-1500 C4) gets a clear target: generate alternative `IInferencePort` strategies, not server-level adapters.
...
**Mitigations:**
- Default `shadow_traffic_fraction` stays at 5% per ADR-2026-04-26-1500 — strategy-level shadow at 5% is roughly the cost of a single per-server miss-route in normal operation.
...
- ADR-2026-04-26-1500 — Substrate (defines `RuntimeComposition`, the port-binding model)
- ADR-2026-04-26-1303 — IModelProvider port + crate split (re-frames `IInferencePort` as the prototype substrate port)
