# ADR-2605172030 — SOP Pipeline Redesign: Tool-Only Artifact Production, Closed-Loop Retries, Structured Classifier
...
**Supersedes:** ADR-2605121505 (Extend SOP Drafter to Emit Non-file_write Action Kinds — the drafter expansion that didn't help)
**Amends:** ADR-2605082400 (Personas as Commitment-Creators), ADR-2605082300 (Digital-Twin Auto-Validator)
**Keeps:** ADR-2605082500 (Typed Tool Library + SOP Execution — this is the **working path**; we are doubling down on it)
...
- `24578` → `ADR-260512-structural-smell.md` — 54 retries
...
**`hex-nexus/src/orchestration/twin_reviewer.rs` — SHRINK.** Tool actions already auto-approved (ADR-2605082500). The remaining branch (free-form drafter actions) goes away with drafter deletion. Twin only judges operator-passthrough writes for sanity (typo prevention) and non-source markdown stubs.
...
- Twin shrinkage means losing some defensive depth on operator-passthrough writes. Mitigation: keep the cargo_check inline gate (ADR-2605110700) which has already saved one .rs typo this session.
...
- ADR-2605121505 (Drafter extension): mark superseded with link to this ADR.
- ADR-2605082400 (Personas as Commitment-Creators): amend to clarify "creators" means *tool-plan creators*, not *file-content creators*.
...
- ADR-2605082500 — Typed Tool Library + SOP Execution (the path we're doubling down on)
- ADR-2605131500 — Fail-open twin judge (informs Phase 3 twin shrinkage)
