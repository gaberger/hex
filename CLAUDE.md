...
11. **Persona work routes through the factory, not ad hoc manual edits.** (Updated per ADR-2606061359/ADR-2606071340 P0 — the always-on multi-agent org-sim and its SOP persona-dispatch path are retired; empirical driver was SOP dispatch silently not engaging while routing, persisting, and never acting on a board ask.) When a persona-domain change is needed (security audit, ADR draft, spec, refactor, even a one-const code change), send the ask via `/api/org/send-message` (or the Missions dashboard compose box — Mission Control itself was retired 2026-06-04 and its dead views were removed from the codebase) and route it through the single-agent gateway loop (`hex do` / the direct executor) rather than editing by hand. The ask must never be silently accepted, persisted, and left unprocessed: either a live functional-role agent (coder/reviewer/tester/planner, spawned per-task and reaped on completion) picks it up and acts, or it is explicitly rejected/escalated to the operator with a visible error/notification. Bypassing this with manual edits is theater — the operator wants to see the factory working, not you.
