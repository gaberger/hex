...
mod real {
    ...
    impl IInferenceTaskStatePort for SpacetimeStateAdapter {
        ...
        async fn inference_task_gate(&self, id: &str, updated_at: &str) -> Result<(), StateError> {
            self.call_reducer("inference_task_gate", serde_json::json!([id, updated_at])).await?;
            Ok(())
        }
    ...
    impl IProjectStatePort for SpacetimeStateAdapter {
        async fn project_register(&self, project: ProjectRegistration) -> Result<(), StateError> {
            ...
            self.call_reducer("register_project", serde_json::json!([
                project.id, project.name, project.description, project.root_path, registered_at, project.ast_is_stub
            ])).await?;
            Ok(())
        }
    ...
    impl IHexAgentStatePort for SpacetimeStateAdapter {
        ...
        async fn hex_agent_update_capabilities(&self, id: &str, capabilities_json: &str) -> Result<(), StateError> {
            let now = chrono::Utc::now().to_rfc3339();
            self.call_reducer("agent_update_capabilities", serde_json::json!([id, capabilities_json, now])).await?;
            Ok(())
        }
...
mod stub {
    ...
    impl IInferenceTaskStatePort for SpacetimeStateAdapter {
        ...
        async fn inference_task_gate(&self, _: &str, _: &str) -> Result<(), StateError> { Err(Self::err()) }
    }
