...
# Add to PATH
cp target/release/hex /usr/local/bin/
```

**Requirements:** [Rust](https://rustup.rs/) (stable), [Bun](https://bun.sh/) (for TypeScript tests), [SpacetimeDB](https://spacetimedb.com/) (for coordination)
...
| `hex analyze <path>` | Architecture health check (boundaries, dead code, ADR compliance) |
| `hex status` | Project status overview |
| `hex mcp` | Start MCP server (stdio transport) for Claude Code / IDE integration |
...
| `hex task create <swarm_id> <title>` | Create a task in a swarm |
| `hex task list` | List all tasks across active swarms |
| `hex task complete <id> [result]` | Mark a task as complete |
| **Agent Management** | |
...
| **Workplans** | |
| `hex plan create <reqs> --adr <ID>` | Decompose requirements into workplan steps |
| `hex plan list` | List existing workplans |
| `hex plan status <file>` | Show workplan status |
| **Testing** | |
| `hex test unit` | Run all unit tests across Rust crates |
| `hex test all` | Full integration suite |
| `hex test history` | Show recent test run history |
| `hex test trends` | Show pass rate trends per category |
| **Infrastructure** | |
| `hex nexus start\|stop\|status` | Manage hex-nexus daemon (dashboard at `:5555`) |
