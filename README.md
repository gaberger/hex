...
The autonomy envelope (ADR-2026-04-27-0800 §1a) is a three-tier table that names exactly which actions the loop may apply without operator consent:
...
- **`hex adr doctor`** scans the ADR registry every tick. Unparseable status → Tier-A auto-fix. Duplicate ID → Tier-C operator review. Stale-Proposed ADR → Tier-B drafted demotion. (ADR-2026-04-27-0800)
- **`hex plan reconcile --strict`** demotes any task whose stored `done` doesn't match the event log. Removes the multi-writer race that produces false-completes. (ADR-2026-04-27-1000)
...
| Evidence-gated `done` | files-exist + workplan-scoped commit-subject match (ADR-2026-04-27-0800 P0) | Other systems store `status: "done"` and trust the writer. hex demotes any task without git evidence. |
...
| Continuous self-improvement | sched-daemon `tick_improver` discovers → proposes → judges → enqueues; ~20 detectors across operational + architectural classes (ADR-2026-04-27-1100, ADR-2026-04-27-1200) | Most "agentic" systems run a loop on user prompts. hex runs a loop without one. |
...
hex adr doctor                # registry health (ADR-2026-04-27-0800)
...
Alpha — but a different kind of alpha than most. Every mechanical claim above has a reproducer in [EVIDENCE.md](docs/EVIDENCE.md): exact command, prerequisites, expected output. The substrate (ADR-2026-04-26-1500), six-layer governance (ADR-2026-04-26-1311), evidence gate (ADR-2026-04-27-0800), workplan state model (ADR-2026-04-27-1000), self-improvement loop (ADR-2026-04-27-1100), and architectural-health detectors (ADR-2026-04-27-1200) are all named and most are partially landed; the chain that closes the operator-asks-nothing loop is the active development frontier. ADR drift, false-done propagation, and detector blind spots are themselves visible in the system as findings the improver will surface — not hidden.
